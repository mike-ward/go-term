...
func (g *Grid) Resize(rows, cols int) {
	...
	// Detach scrollback into stable slices before reflow rebuilds the ring's
	// backing buffer; Row() aliases that buffer.
...
func (g *Grid) ViewportMatchesRegex(re *regexp.Regexp) []SearchMatch {
	...
	for vr := range g.Rows {
		...
		for {
			...
			col = c + max(l, 1) // advance past zero-length matches
		}
	}
	return matches
}
